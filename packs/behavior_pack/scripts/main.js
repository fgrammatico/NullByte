...
// ---------------------------------------------------------------------------
function registerAllObjectives() {
...
function ensureObjectivesRegistered() {
    if (objectivesRegistered)
        return;
...
function clearLegacyNoiseDisplay() {
    try {
...
function ensureSharedStateRegistered() {
...
function runDeferredPlayerCommand(origin, action) {
...
function setScore(objective, value) {
    const scoreboardObjective = world.scoreboard.getObjective(objective);
    if (!scoreboardObjective)
        return;
    try {
...
function getMisuseSpawnCount(level) {
...
function findSafeSpawnLocation(player, x, z) {
...
function getPatrolMarkers(player) {
    ...
    try {
        ...
        const matches = player.dimension.getBlocks(volume, { includeTypes: ["minecraft:chiseled_stone_bricks"] }, true);
...
function applyCommandPenalty(player, reason, baseNoise) {
    ...
    player.sendMessage(`§c[terminal]§r  ${reason} §7(noise +${baseNoise}, patrol +${spawnCount}, now ${updatedNoise}/100)§r`);
}
...
// ---------------------------------------------------------------------------
const lastChatDispatch = new Map();
function dispatchChatCommand(sender, rawMessage) {
    ensureSharedStateRegistered();
    ...
    if (!normalized.toLowerCase().startsWith("nb:"))
        return false;
    ...
    if (lastChatDispatch.get(key) === stamp)
        return true;
...
function registerChatFallbackCommands() {
...
// ---------------------------------------------------------------------------
function registerScriptEventFallbackCommands() {
    const systemAny = system;
    ...
    signal.subscribe((ev) => {
        ...
        const sender = (event.sourceEntity ?? event.initiator);
        if (!(sender instanceof Player))
            return;
...
function handlePortKnock(rawPort) {
    ensureSharedStateRegistered();
    if (getScore(OBJ.p07) >= 1 || getScore(OBJ.victory) >= 1)
        return;
...
function handleHelp(origin) {
    return runDeferredPlayerCommand(origin, (player) => {
        ...
        if (hasExploitToken || (hasPortKnock && coreDefenseDown)) {
            ...
            if (hasExploitToken)
                lines.push("    §8›§r firewall" + (isUser ? "" : " §8[user required]§r"));
...
function handleWhoami(origin) {
    return runDeferredPlayerCommand(origin, (player) => {
        ensureSharedStateRegistered();
        const permission = getScore(OBJ.perm);
        let role;
        if (permission >= PERM_ROOT)
            role = "§aroot§r";
        else if (permission >= PERM_ADMIN)
            role = "§cadmin§r";
        else if (permission >= PERM_USER)
            role = "§euser§r";
        else
            role = "§8guest§r";
        const uid = Math.abs(player.name.split("").reduce((acc, ch) => acc + ch.charCodeAt(0), 0) % 9999);
        player.sendMessage(`§7[whoami]§r  operator=${player.name}  shared_role=${role}  uid=${uid}`);
    });
}
function handleScan(origin) {
    return runDeferredPlayerCommand(origin, (player) => {
        ensureSharedStateRegistered();
...
function handleStatus(origin) {
    return runDeferredPlayerCommand(origin, (player) => {
        ...
        player.sendMessage(`§7[status]§r  Challenges: §a${solved}/5§r  Noise: ${noiseBar(noise)} ${noise}  Alarms: §c${alarms}§r`);
    });
}
...
function handleLogin(origin, ...args) {
    return runDeferredPlayerCommand(origin, (player) => {
        ensureSharedStateRegistered();
        ...
        const params = args.filter((a) => typeof a === "string");
        if (params.length < 2) {
...
function handleSudo(origin, ...args) {
    return runDeferredPlayerCommand(origin, (player) => {
        ensureSharedStateRegistered();
        const params = args.filter((a) => typeof a === "string");
        if (params.length < 1) {
...
function handleCat(origin, ...args) {
    return runDeferredPlayerCommand(origin, (player) => {
        ensureSharedStateRegistered();
        if (getScore(OBJ.p01) < 1) {
...
function handleExploit(origin, ...args) {
    return runDeferredPlayerCommand(origin, (player) => {
        ensureSharedStateRegistered();
        const params = args.filter((a) => typeof a === "string");
        if (params.length < 1) {
...
function handleKillPatrol(origin) {
    return runDeferredPlayerCommand(origin, (player) => {
        ensureSharedStateRegistered();
        if (getScore(OBJ.perm) < PERM_ADMIN) {
        ...
        for (const entity of player.dimension.getEntities({
            ...
            if (entity instanceof Player)
                continue;
            if (!defenseTypes.has(entity.typeId))
                continue;
            try {
                entity.kill();
                killed++;
...
const lastFlagState = new Map();
let flagBaselineSeeded = false;
function announceFlagGains() {
    for (const key of FLAG_KEYS) {
        ...
        if (flagBaselineSeeded) {
            ...
            if (prev < 1 && value >= 1) {
                ...
                addNoise(-3);
                world.sendMessage(`§a[FLAG CAPTURED]§r  §e${meta.name}§r unlocked! §7${meta.hint} Noise -3.§r`);
                for (const player of world.getAllPlayers()) {
...
function spawnSharedPatrols(count, band) {
...
function onBandEscalation(from, to) {
    if (from === to)
        return;
    const bandRank = { CLEAN: 0, WARNING: 1, ALERT: 2, BREACH: 3, LOCKDOWN: 4 };
...
function checkDimensionEntry(player) {
...
function gameTick() {
    ...
    for (const player of players) {
        if (lockTicks > 1) {
            ...
            player.onScreenDisplay.setActionBar(`§7[shared noise]§r ${noiseBar(noise)} §f${noise}/100 §7${band}§r`);
        }
...
// ---------------------------------------------------------------------------
function enforceBoundary(player) {
    if (player.dimension.id !== "minecraft:overworld")
        return;
    const loc = player.location;
    const out = loc.x < BOUNDARY.minX ||
        loc.x > BOUNDARY.maxX ||
        loc.z < BOUNDARY.minZ ||
        loc.z > BOUNDARY.maxZ;
    if (out) {
...
world.afterEvents.entityHitEntity.subscribe((event) => {
    if (!(event.damagingEntity instanceof Player))
        return;
    if (!DEFENSE_MOB_IDS.has(event.hitEntity.typeId))
        return;
    ensureSharedStateRegistered();
    addNoise(3);
});
...
world.afterEvents.playerBreakBlock.subscribe((event) => {
    if (!SERVER_HARDWARE_BLOCK_IDS.has(event.brokenBlockPermutation.type.id))
        return;
    ensureSharedStateRegistered();
    addNoise(5);
